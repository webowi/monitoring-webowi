...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain category
      decision: data-trapped + missing-capability + decision-paralysis (workflow-friction not primary)
    - topic: insight
      decision: Symfony-bundle ergonomics — `composer require` + minimal config, matching the way Symfony devs already expect things to work
    - topic: primary persona scope
      decision: solo Symfony developer running side-projects / small client apps
    - topic: Symfony app auth to monitoring service
      decision: per-project API key, generated in the web UI and pasted into Symfony bundle config
    - topic: human auth to web UI
      decision: email + password sign-up and sign-in (NOTE — see MVP scope-down: public sign-up moves to v2; MVP pre-seeds a single user)
    - topic: roles
      decision: flat — a user account owns one or more projects; per-project isolation; no admin/member/viewer roles in MVP
    - topic: MVP scope-down
      decision: drop packaged Symfony bundle (replace with copy-pasted Monolog HttpHandler snippet); drop public sign-up + project CRUD UI (pre-seed one user + one project + one API key); ship only the HTTP error-code filter
    - topic: MVP timeline
      decision: 3 weeks (after scope-down); longer flow would have been 4–6+ weeks
  frs_drafted: 9
  quality_check_status: accepted
---

# Shape notes — Monitoring Webowi

## Seed (verbatim from idea.notes.md)

> Główny problem: Brak darmowego narzędzia do pobierania logów z aplikacji dla programistów, które można bardzo łatwo skonfigurować i było bardzo czytelne oraz łatwe w obłudze.
>
> Najmniejszy zestaw funkcjonalności:
> - Łatwa konfiguracja podpięcia wysyłania logów do aplikacji dla frameworku Symfony
> - Możliwość filtrowania po kodach błędu
> - Przeglądanie przesłanych logów
> - Prosty system kont użytkownika
>
> Co NIE wchodzi w zakres MVP:
> - Podpięcie większej ilości aplikacji do wysyłania logów
> - Komunikaty mailowe, sms w przypadku błędów krytycznych
> - Wykresy błędów
>
> Kryteria sukcesu:
> - Logi z aplikacji na frameworku Symfony trafiają do aplikacji Monitoring Webowi
> - Istnieje możliwość filtracji po kodach błędu
...
## MVP scope-down record

The following capabilities were intentionally cut from MVP and moved to v2+, to keep the first shippable version inside ~3 weeks of after-hours work:

- **Packaged Symfony bundle** — v1 ships a copy-pasted Monolog `HttpHandler` snippet instead of `composer require monitoring-webowi/symfony-bundle`. The bundle becomes v2 once ingestion + UI are proven.
- **Public sign-up & multi-tenant UI** — v1 has no sign-up flow. The single user account is pre-seeded (DB row or env). Sign-up + password reset move to v2.
- **Project CRUD UI** — v1 has one pre-seeded project with one pre-seeded API key. The "create project / regenerate key" UI moves to v2.
- **Multiple filters** — v1 ships only two filter axes (Monolog severity level + HTTP status code, per FR-008's revised scope). Date range, free-text search across message bodies, and grouping/clustering by error pattern move to v2.

These cuts preserve the Primary + Secondary outcomes (Symfony app → UI, filter by code, near-real-time post-deploy visibility) and do not weaken either Guardrail.
...
1. **Grouping in v2.** Deferred from FR-007 Socratic round. Grouping the log list by error pattern (exception class, message template, URL pattern) is useful during a 500-spike but is not in MVP. The v1 data model should be chosen so v2 grouping is mechanical rather than a rewrite. Owner: user. Resolution timing: during v2 planning.
...
3. **Bundle vs. snippet in v2.** Deferred from MVP scope-down. MVP ships a paste-in Monolog snippet; v2 should re-evaluate whether to publish a real Symfony bundle to Packagist. Owner: user.
4. **Per-project event volume cap.** No upper bound is currently set on events/sec or events/day per project. If a misconfigured Symfony app suddenly ships logs at very high rates, ingestion behavior under load is undefined. Owner: user. Resolution timing: before /10x-tech-stack-selector hardens storage choice.
5. **Product type hybrid.** Selected `product_type: web-app`, but the same deployed product also exposes a machine-facing ingestion HTTP endpoint that the Symfony bundle/snippet calls. Treated at PRD level as one product with two surfaces (human web UI + machine ingestion route), not two separate products. Owner: user. Resolution timing: confirm during /10x-tech-stack-selector when picking the request-handling surface.
...
- **No log grouping / clustering by error pattern.** Already deferred via FR-007 Socratic round (in Open Questions); explicitly locked out of MVP here so it does not creep back in.
- **No project sharing with a second user, team workspaces, or per-project role matrices.** The flat single-owner-per-project model from Phase 2 is binding for MVP.
