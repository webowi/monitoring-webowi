...
namespace App\Tests\Unit\Projects\Application\GenerateIngestionKey;
...
class GenerateIngestionKeyHandlerTest extends TestCase
{
    ...
    private GenerateIngestionKeyHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new GenerateIngestionKeyHandler(
    ...
    private function buildProject(Uuid $organizationId): Project
    {
    ...
    private function buildActiveKey(Uuid $projectId): IngestionKey
    {
        return IngestionKey::new($projectId, 'Existing Key', 'existingHash', 'mon_ing_existing0000000000000000000000');
    }

    #[Test]
    public function generatesFirstKeyWhenNoneExists(): void
    {
        $orgId = Uuid::v4();
        $projectUuid = Uuid::v4();
        $project = $this->buildProject($orgId);
    ...
    #[Test]
    public function throwsWhenActiveKeyAlreadyExists(): void
    {
        $orgId = Uuid::v4();
        $projectUuid = Uuid::v4();
        $project = $this->buildProject($orgId);
        $user = $this->buildUser($orgId);
        $existingKey = $this->buildActiveKey($projectUuid);

        $this->projectRepository->method('getById')->willReturn($project);
        $this->currentUserFetcher->method('fetchUser')->willReturn($user);
        $this->ingestionKeyRepository->method('findActiveByProjectId')->willReturn($existingKey);
        $this->ingestionKeyRepository->expects($this->never())->method('save');

        $this->expectException(IngestionKeyAlreadyExistsException::class);
    ...
    {
        $projectUuid = Uuid::v4();
        $project = $this->buildProject(Uuid::v4());
