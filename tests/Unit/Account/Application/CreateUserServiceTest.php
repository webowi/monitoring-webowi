...
class CreateUserServiceTest extends TestCase
{
    ...
    public function testForceVerificationWhenFlagProvided(): void
    {
        $user = new User();
        ...
        $this->userRepository
            ->expects($this->once())
            ->method('save')
            ->with($this->callback(fn (User $user) => true === $user->isVerified()));
        $this->accountMailerService
            ->expects($this->never())
            ->method('sendRegistrationConfirmationEmail');
        $this->entityManager
        ...
        $this->service->createUser('email', 'password', true, true);
    }
}
